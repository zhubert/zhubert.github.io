...
I've spent the last few months building two educational projects to understand how transformer models work. Not just surface-level understanding, but the kind where you can explain every calculation, every design choice, every gradient flowing backward through the network.
...
Large language models like GPT, Claude, and others have become essential tools for developers and researchers. But understanding how they actually work - not just calling an API, but knowing what happens when you feed text into a transformer - requires getting your hands dirty with the mathematics and code.

I wanted to move beyond treating transformers as black boxes. So I built them from scratch, documented every component, and calculated every derivative by hand.
...
Every component includes extensive documentation explaining not just what it does, but why it's designed that way. The code prioritizes clarity over performance - this is for learning, not production.
...
The documentation is built with Astro/Starlight and includes interactive math rendering, color-coded matrices, and Python scripts you can run to verify every calculation yourself.

## What I Learned

Building these projects taught me things I wouldn't have learned any other way:
...
The transformer implementation lets you build something real. You can train it on actual data, generate text, and use interpretability tools to understand what it learned. It's practical and complete.

The manual calculations force you to understand the mathematics deeply. You can't hand-wave away a Jacobian matrix when you're calculating every element by hand. It's rigorous and foundational.
...
Both projects are on GitHub with complete documentation:

- [Transformer](https://github.com/zhubert/transformer) - Full PyTorch implementation with training, generation, and interpretability
- [Attention to Detail](https://github.com/zhubert/attention-to-detail) - Step-by-step manual calculations

If you're interested in understanding how modern LLMs work, I'd suggest starting with the transformer project's documentation. Read through the core components in order: attention, embeddings, feedforward, blocks, model. Then try the manual calculations to see exactly what's happening at each step.

The code and calculations speak for themselves. These are learning resources, built to prioritize understanding over everything else.
